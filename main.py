...
class BugHunterApp(MDApp):
    def build(self):
        ...
        self.left_rail = MDBoxLayout(
            ...
            md_bg_color=(0, 0, 0, 1), padding=[0, dp(10), 0, 0]
        )
        self.menu_btn = MDIconButton(
            icon="menu", theme_text_color="Custom", text_color=(1, 1, 1, 1),
    ...
    def add_log(self, message):
        Clock.schedule_once(lambda dt: setattr(
            self.log_text, 'text', self.log_text.text + f"[*] {message}\n"
        ))
    ...
    def run_scanner(self):
        ...
        try:
            ...
            if not portal:
                self.add_log("No captive portal found. Are you on a restricted network?")
                return
            ...
            for m in matches:
                ...
                if not re.search(EXCLUDE_FILTER, m, re.IGNORECASE):
                    ...
                    if self.handshake_verify(m):
                        self.selected_bug = m
                        self.add_log(f"BUG FOUND: {m} (101 Switching Protocols)")
                        break
            else:
                self.add_log("Scan complete. No working bug found.")
    ...
    def handshake_verify(self, bug):
        ...
        try:
            ...
            ss.send(h.encode())
            res = ss.recv(1024).decode(errors='ignore')
            ss.close()
            return "101" in res
        except Exception:
            return False
    ...
    def android_service(self, action):
        try:
            ...
            VpnEngine  = autoclass('org.test.bughunter.VpnEngine')

            if action == "START":
                prepare_intent = VpnService.prepare(activity)
                if prepare_intent:
                    activity.startActivityForResult(prepare_intent, 0)
                activity.startService(Intent(activity, VpnEngine))
            else:
                si = Intent(activity, VpnEngine)
                si.setAction("STOP")
                activity.startService(si)

        except Exception as e:
            self.add_log(f"Android API Error: {e}")
        ...
        try:
            ...
            async with aiohttp.ClientSession() as sess:
                async with sess.ws_connect(
                    f"https://{CONNECT_IP}:{PORT}{WS_PATH}",
                    headers=ws_headers,
                    ssl=ssl_ctx,
                    server_hostname=self.selected_bug
                ) as ws:
                    Clock.schedule_once(lambda dt: setattr(self.status_label, 'text', 'CONNECTED'))
                    self.add_log(f"Tunnel: {addr}:{port} | SNI: {self.selected_bug}")
                    ...
                    async def upstream():
                        ...
                        while self.connected:
                            ...
                            if not data:
                                break
                            if first:
                                header = struct.pack("!B16sBBH", 0, uuid.UUID(USER_UUID).bytes, 0, 1, port)
                                if atyp == 1:
                                    header += b"\x01" + socket.inet_aton(addr)
                                else:
                                    header += b"\x02" + len(addr).to_bytes(1, 'big') + addr.encode()
                                await ws.send_bytes(header + data)
                                first = False
                            else:
                    ...
                    async def downstream():
                        ...
                        async for msg in ws:
                            ...
                            if msg.type == aiohttp.WSMsgType.BINARY:
                                payload = msg.data[2:] if first_res else msg.data
